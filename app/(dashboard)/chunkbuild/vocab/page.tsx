// app/(dashboard)/chunkbuild/vocab/page.tsx
...
function VocabChunkView({ chunk }: { chunk: VocabChunk }) {
  return (
    ...
    >
      ...
      >
        Level: {chunk.data.level} · {chunk.data.items.length} words
      </p>

      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {chunk.data.items.map((item: VocabItem, idx) => (
          <li
            key={`${item.word}-${idx}`}
            style={{
              padding: '10px 0',
          ...
          >
            ...
            >
              <strong style={{ fontSize: 12, color: '#6b7280' }}>예문:</strong>{' '}
              {item.example_en}
            </p>
          </li>
        ))}
...
export default function VocabChunkBuildPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setProgress(
        `총 ${sentences.length}개의 문장을 기반으로 VocabChunk를 생성합니다...`,
      );

      const res = await fetch('/api/chunkbuild/vocabChunk', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'VocabChunk API 호출 중 오류가 발생했습니다.');
      }

      const data = await res.json();
      const receivedChunk: VocabChunk | undefined = data.chunk;
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>
        SentenceRecord[] → VocabChunk 변환 테스트
      </h1>
      <p style={{ color: '#555', marginBottom: 24 }}>
        1차 파이프라인(processUserMaterial)에서 생성된 JSON 배열
        (SentenceRecord[])을 아래에 붙여넣은 뒤,&nbsp;
        <b>VocabChunk</b> 생성 결과를 확인할 수 있는 개발용 페이지입니다.
      </p>

      {/* 옵션 영역: level + maxItems */}
      <section
        style={{
          marginBottom: 16,
          display: 'flex',
      ...
      <section style={{ marginTop: 32 }}>
        ...
        {loading && (
          <p style={{ color: '#555', fontSize: 14 }}>
            서버에서 VocabChunk를 생성 중입니다…
          </p>
        )}
        {chunk && <VocabChunkView chunk={chunk} />}
      </section>
