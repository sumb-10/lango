// app/(dashboard)/chunkbuild/background/page.tsx
...
export default function BackgroundChunkBuildPage() {
  const [rawInput, setRawInput] = useState('');
  const [level, setLevel] = useState<CEFRLevel>('C1');
  const [maxParagraphs, setMaxParagraphs] = useState<number>(3);
  const [chunk, setChunk] = useState<BackgroundChunk | null>(null);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setProgress(
        `총 ${sentences.length}개의 문장을 기반으로 BackgroundChunk를 생성합니다...`,
      );

      const res = await fetch('/api/chunkbuild/backgroundChunk', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.error || 'BackgroundChunk API 호출 중 오류가 발생했습니다.',
        );
      }

      const data = await res.json();
      const receivedChunk: BackgroundChunk | undefined = data.chunk;
      ...
      setProgress('BackgroundChunk 생성 완료.');
    } catch (err: any) {
      console.error(err);
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>
        SentenceRecord[] → BackgroundChunk 변환 테스트
      </h1>
      <p style={{ color: '#555', marginBottom: 24 }}>
        1차 파이프라인(processUserMaterial)에서 생성된 JSON 배열
        (SentenceRecord[])을 아래에 붙여넣은 뒤,&nbsp;
        <b>BackgroundChunk</b> 생성 결과를 확인할 수 있는 개발용 페이지입니다.
        <br />
      ...
      >
        ...
        <label style={{ fontSize: 13 }}>
          ...
          <input
            ...
            max={8}
            value={maxParagraphs}
            onChange={(e) => setMaxParagraphs(Number(e.target.value) || 1)}
            style={{
              width: 70,
              padding: '4px 6px',
              borderRadius: 6,
      ...
      >
        ...
        >
          {loading ? '생성 중…' : 'BackgroundChunk 생성하기'}
        </button>
        {error && (
      ...
      <section style={{ marginTop: 32 }}>
        ...
        {loading && (
          <p style={{ color: '#555', fontSize: 14 }}>
            서버에서 BackgroundChunk를 생성 중입니다…
          </p>
        )}
        {chunk && <BackgroundChunkView chunk={chunk} />}
      </section>
